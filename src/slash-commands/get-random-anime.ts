...
export const GetRandomAnimeCommand: SlashCommand = {
  ...
  async run(interaction) {
    ...
    const response = await axios.get<GetRandomAnimeResponse['data']>(GET_RANDOM_ANIME_URL, {
    ...
    if (!response || response.status !== 200) {
      await interaction.editReply({
        embeds: [
          new MessageEmbed()
            .setAuthor({ name: `Response failed` })
            .setDescription(`Error ${response.status}: ${response.statusText}`)
            .setColor('RED')
            .setFooter({ text: `${response.config.method?.toUpperCase()}: ${response.config.url}` })
            .setFields([]),
        ],
      });
      return;
    ...
    addField('Scored By', scored_by);
    addField('Rating', rating);
    addField('Duration', duration);

    const genresString = genres.map(({ name, url }) => hyperlink(name, url)).join('\n');
    const themesString = themes.map(({ name, url }) => hyperlink(name, url)).join('\n');
